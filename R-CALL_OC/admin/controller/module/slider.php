...
class ControllerModuleSlider extends Controller {
	...
	public function index() {
		$this->load->language('module/slider'); //подключаем наш языковой файл
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { //если мы нажали "Сохранить"  в панели, мы сохраняем текущие настройки
			$this->model_setting_setting->editSetting('slider', $this->request->post);
		...
		$data['action'] = $this->url->link('module/slider', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/slider', 'token=' . $this->session->data['token'], 'SSL');

		//переменная с статусом модуля
        if (isset($this->request->post['slider_status'])) {
			$data['slider_status'] = $this->request->post['slider_status'];
		} else {
			$data['slider_status'] = $this->config->get('slider_status');
		}
		...
		$this->response->setOutput($this->load->view('module/slider.tpl', $data));
	}
