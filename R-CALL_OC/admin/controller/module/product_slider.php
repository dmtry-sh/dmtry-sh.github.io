<?php
class ControllerModuleProductSlider extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('module/product_slider'); //подключаем наш языковой файл
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { //если мы нажали "Сохранить"  в панели, мы сохраняем текущие настройки
			$this->model_setting_setting->editSetting('product_slider', $this->request->post);
		...
		$data['action'] = $this->url->link('module/product_slider', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		//переменная с статусом модуля
        if (isset($this->request->post['product_slider_status'])) {
			$data['product_slider_status'] = $this->request->post['product_slider_status'];
		} else {
			$data['product_slider_status'] = $this->config->get('product_slider_status');
		}
        ...
        //в качестве файла представления модуля для панели администратора использовать файл product_slider.tpl
		$this->response->setOutput($this->load->view('module/product_slider.tpl', $data));
	}
