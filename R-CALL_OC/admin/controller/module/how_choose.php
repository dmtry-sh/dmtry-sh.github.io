<?php
class ControllerModuleHowChoose extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('module/how_choose'); //подключаем наш языковой файл
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { //если мы нажали "Сохранить"  в панели, мы сохраняем текущие настройки
			$this->model_setting_setting->editSetting('how_choose', $this->request->post);
		...
		$data['action'] = $this->url->link('module/how_choose', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		//переменная с статусом модуля
        if (isset($this->request->post['how_choose_status'])) {
			$data['how_choose_status'] = $this->request->post['how_choose_status'];
		} else {
			$data['how_choose_status'] = $this->config->get('how_choose_status');
		}
        ...
        //в качестве файла представления модуля для панели администратора использовать файл how_choose.tpl
		$this->response->setOutput($this->load->view('module/how_choose.tpl', $data));
	}
