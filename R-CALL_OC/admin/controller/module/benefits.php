<?php
class ControllerModuleBenefits extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('module/benefits'); //подключаем наш языковой файл
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { //если мы нажали "Сохранить"  в панели, мы сохраняем текущие настройки
			$this->model_setting_setting->editSetting('benefits', $this->request->post);
		...
		$data['action'] = $this->url->link('module/benefits', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		//переменная с статусом модуля
        if (isset($this->request->post['benefits_status'])) {
			$data['benefits_status'] = $this->request->post['benefits_status'];
		} else {
			$data['benefits_status'] = $this->config->get('benefits_status');
		}
        ...
        //в качестве файла представления модуля для панели администратора использовать файл benefits.tpl
		$this->response->setOutput($this->load->view('module/benefits.tpl', $data));
	}
