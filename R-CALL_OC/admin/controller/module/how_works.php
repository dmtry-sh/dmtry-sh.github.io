<?php
class ControllerModuleHowWorks extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('module/how_works'); //подключаем наш языковой файл
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { //если мы нажали "Сохранить"  в панели, мы сохраняем текущие настройки
			$this->model_setting_setting->editSetting('how_works', $this->request->post);
		...
		$data['action'] = $this->url->link('module/how_works', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		//переменная с статусом модуля
        if (isset($this->request->post['how_works_status'])) {
			$data['how_works_status'] = $this->request->post['how_works_status'];
		} else {
			$data['how_works_status'] = $this->config->get('how_works_status');
		}
        ...
        //в качестве файла представления модуля для панели администратора использовать файл how_works.tpl
		$this->response->setOutput($this->load->view('module/how_works.tpl', $data));
	}
