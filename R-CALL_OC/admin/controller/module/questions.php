<?php
class ControllerModuleQuestions extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('module/questions'); //подключаем наш языковой файл
		...
		$data['action'] = $this->url->link('module/questions', 'token=' . $this->session->data['token'], 'SSL');
		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		//переменная с статусом модуля
        if (isset($this->request->post['questions_status'])) {
			$data['questions_status'] = $this->request->post['questions_status'];
		} else {
			$data['questions_status'] = $this->config->get('questions_status');
		}
        ...
        //в качестве файла представления модуля для панели администратора использовать файл questions.tpl
		$this->response->setOutput($this->load->view('module/questions.tpl', $data));
	}
